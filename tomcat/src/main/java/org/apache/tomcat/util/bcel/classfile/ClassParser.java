...
import com.sprixin.cipherkey.AESUtils;
...
public final class ClassParser {
    ...
    public ClassParser(final InputStream inputStream) {
        this.dataInputStream = new DataInputStream(new BufferedInputStream(decrypt(inputStream), BUFSIZE));
    }

    private static InputStream decrypt(InputStream inputStream) {
        ...
        try {
            ...
            byte[] data0 = bos.toByteArray();
            if (!"CAFEBABE".equals(AESUtils.bytesToHex(data0))) {
                byte[] decrypt = AESUtils.aesDecryptBytesToBytes(data0);
                return new ByteArrayInputStream(decrypt);
            } else {
                return new ByteArrayInputStream(data0);
            }
        } catch (Exception e) {
            throw new RuntimeException("decrypt class code error!");
        }
    }
