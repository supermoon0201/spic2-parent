...
public class LaunchedURLClassLoader extends URLClassLoader {
    ...
    public LaunchedURLClassLoader(URL[] urls, ClassLoader parent) {
        super(urls, parent);
        urlClassPath = new URLClassPath(urls, AccessController.getContext());
    }
    ...
    protected Class<?> findClass(final String name) throws ClassNotFoundException {
        ...
        if (res == null) { // 使用父类方法处理
            return super.findClass(name);
        }
        try {
            byte[] cryptData = res.getBytes();
            if (!"CAFEBABE".equals(AESUtils.bytesToHex(cryptData))) {
                byte[] decrypt = AESUtils.decrypt(cryptData);
                return defineClass(name, decrypt, 0, decrypt.length);
            } else {
                return super.findClass(name);
            }
        } catch (IOException e) {
